...
class Mamba3Block(nn.Module):
    ...
    def __init__(self, d_model: int, ffn_dim: int, d_state: int, headdim: int, norm_eps: float, residual_scale: float, layer_idx: int):
        super().__init__()
        self.residual_scale = residual_scale
        self.attn_norm = RMSNorm(d_model, norm_eps)
        self.mixer = _official_mamba(d_model=d_model, d_state=d_state, headdim=headdim, chunk_size=64, layer_idx=layer_idx)
